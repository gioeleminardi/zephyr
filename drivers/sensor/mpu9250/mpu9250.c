...
static void mpu9250_convert_accel(struct sensor_value *val, int16_t raw_val, uint16_t sensitivity_shift)
{
  int64_t conv_val;
...
static void mpu9250_convert_gyro(struct sensor_value *val, int16_t raw_val, uint16_t sensitivity_x10)
{
  int64_t conv_val;
...
static inline void mpu9250_convert_temp(struct sensor_value *val, int16_t raw_val)
{
  ...
  val->val2 = (((int64_t) (raw_val % MPU0259_TEMP_SENSITIVITY) * 1000000) / MPU0259_TEMP_SENSITIVITY);
                               ...
                               struct sensor_value *val)
{
  ...
  switch (chan)
  {
    case SENSOR_CHAN_ACCEL_XYZ:
      ...
      mpu9250_convert_accel(val + 1, drv_data->accel_y, drv_data->accel_sensitivity_shift);
      mpu9250_convert_accel(val + 2, drv_data->accel_z, drv_data->accel_sensitivity_shift);
      break;
    ...
    case SENSOR_CHAN_MAGN_XYZ:
      ret = ak8963_convert_magn(val, drv_data->magn_x, drv_data->magn_scale_x, drv_data->magn_st2);
      if (ret < 0)
      {
        return ret;
      }
      ret = ak8963_convert_magn(val + 1, drv_data->magn_y, drv_data->magn_scale_y, drv_data->magn_st2);
      if (ret < 0)
      {
        return ret;
      }
      ret = ak8963_convert_magn(val + 2, drv_data->magn_z, drv_data->magn_scale_z, drv_data->magn_st2);
      return ret;
    case SENSOR_CHAN_MAGN_X:
      return ak8963_convert_magn(val, drv_data->magn_x, drv_data->magn_scale_x, drv_data->magn_st2);
    case SENSOR_CHAN_MAGN_Y:
      return ak8963_convert_magn(val, drv_data->magn_y, drv_data->magn_scale_y, drv_data->magn_st2);
    case SENSOR_CHAN_MAGN_Z:
      return ak8963_convert_magn(val, drv_data->magn_z, drv_data->magn_scale_z, drv_data->magn_st2);
    case SENSOR_CHAN_DIE_TEMP: mpu9250_convert_temp(val, drv_data->temp); break;
#endif
...
static int mpu9250_calibrate(const struct device *dev)
{
  ...
  ret = i2c_reg_write_byte_dt(&cfg->i2c,
                              MPU9250_REG_FIFO_EN,
                              0x78);// Enable gyro and accelerometer sensors for FIFO  (max size 512
                                    // bytes in MPU-9150)
  if (ret < 0)
  {
  ...
  // upon device startup
  calibData[0] = (-gyro_bias[0] / 4 >> 8) & 0xFF;// Divide by 4 to get 32.9 LSB per deg/s to conform
                                                 // to expected bias input format
  calibData[1] = (-gyro_bias[0] / 4) & 0xFF;// Biases are additive, so change sign on calculated
                                            // average gyro biases
  ...
  LOG_DBG("Gyro bias X: %f, Gyro bias Y: %f, Gyro bias Z: %f",
          drv_data->gyroBias[0],
          drv_data->gyroBias[1],
          drv_data->gyroBias[2]);
  ...
  calibData[1] = calibData[1] | mask_bit[0];// preserve temperature compensation bit when writing
                                            // back to accelerometer bias registers
  calibData[2] = (accel_bias_reg[1] >> 8) & 0xFF;
  calibData[3] = (accel_bias_reg[1]) & 0xFF;
  calibData[3] = calibData[3] | mask_bit[1];// preserve temperature compensation bit when writing
                                            // back to accelerometer bias registers
  calibData[4] = (accel_bias_reg[2] >> 8) & 0xFF;
  calibData[5] = (accel_bias_reg[2]) & 0xFF;
  calibData[5] = calibData[5] | mask_bit[2];// preserve temperature compensation bit when writing
                                            // back to accelerometer bias registers
  ...
  LOG_DBG("Accel bias X: %f, Accel bias Y: %f, Accel bias Z: %f",
          drv_data->accelBias[0],
          drv_data->accelBias[1],
          drv_data->accelBias[2]);
...
static int mpu9250_init(const struct device *dev)
{
  ...
  ret = i2c_reg_write_byte_dt(&cfg->i2c, MPU9250_REG_GYRO_CFG, cfg->gyro_fs << MPU9250_GYRO_FS_SHIFT);
  if (ret < 0)
  {
  ...
  ret = i2c_reg_write_byte_dt(&cfg->i2c, MPU9250_REG_ACCEL_CFG2, cfg->gyro_dlpf);
  if (ret < 0)
  {
...
#define INIT_MPU9250_INST(inst)                                                                    \
  ...
  static const struct mpu9250_config mpu9250_cfg_##inst                                            \
      = {.i2c         = I2C_DT_SPEC_INST_GET(inst),                                                \
         ...
         IF_ENABLED(CONFIG_MPU9250_TRIGGER, (.int_pin = GPIO_DT_SPEC_INST_GET(inst, irq_gpios)))}; \
                                                                                                   \
