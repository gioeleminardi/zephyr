...
static int ak8963_execute_rw(const struct device *dev, uint8_t reg, bool write)
{
  ...
  ret = i2c_reg_write_byte_dt(&cfg->i2c, MPU9250_REG_I2C_SLV4_ADDR, AK8963_I2C_ADDR | mode_bit);
  if (ret < 0)
  {
  ...
  /* Initiate transfer  */
  ret = i2c_reg_write_byte_dt(&cfg->i2c, MPU9250_REG_I2C_SLV4_CTRL, MPU9250_REG_I2C_SLV4_CTRL_VAL);
  if (ret < 0)
  {
...
static int ak8963_fetch_adj(const struct device *dev)
{
  ...
  LOG_DBG("Adjustment values %d %d %d",
...
static int ak8963_init_master(const struct device *dev)
{
  ...
  ret = i2c_reg_write_byte_dt(
      &cfg->i2c, MPU9250_REG_USER_CTRL, MPU9250_REG_USER_CTRL_I2C_MASTERMODE_VAL);
  if (ret < 0)
  {
  ...
  ret = i2c_reg_write_byte_dt(
      &cfg->i2c, MPU9250_REG_I2C_MST_CTRL, MPU9250_REG_I2C_MST_CTRL_WAIT_MAG_400KHZ_VAL);
  if (ret < 0)
  {
...
static int ak8963_init_readout(const struct device *dev)
{
  ...
  /* Set target i2c address */
  ret = i2c_reg_write_byte_dt(&cfg->i2c, MPU9250_REG_I2C_SLV0_ADDR, AK8963_I2C_ADDR | I2C_READ_FLAG);
  if (ret < 0)
  {
  ...
  ret = i2c_reg_write_byte_dt(&cfg->i2c, MPU9250_REG_I2C_SLV0_CTRL, MPU9250_REG_READOUT_CTRL_VAL);
  if (ret < 0)
  {
